...
SAMPLING_RULE = (
    "Не делай выводов о конкретном менеджере, если у него меньше 30 разобранных\n"
    "звонков за рассматриваемый период. Не называй разницу между менеджерами\n"
    "значимой, если она меньше 20 процентных пунктов.\n\n"
    "В этих случаях прямо пиши «данных недостаточно для вывода» и указывай, сколько\n"
    "звонков нужно набрать. Не смягчай, не оговаривайся, не выдавай предположение\n"
    "за вывод."
)
...
Прежде чем написать в ответе ЛЮБОЙ вывод, оценку, сравнение или рекомендацию,
касающуюся КОНКРЕТНОГО менеджера (не отдела в целом) — обязан вызвать
инструмент verify_conclusion с точной формулировкой вывода и данными, на
которые опираешься. Если verdict не confirmed — не пиши этот вывод в ответе
вообще, замени на «данных недостаточно для вывода» и сколько нужно добрать.
Выводы про отдел в целом (не про конкретного человека) verify_conclusion не
требуют.
