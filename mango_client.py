"""
Общие вызовы Mango Office VPBX API — используется demo_bot.py (справочник
сотрудников) и, позже, worker.py (запрос ссылки на запись).
...
# Общий клиент на процесс, а не новый на каждый вызов. worker.py опрашивает
# Mango раз в 2 минуты — за 20 часов набегает больше тысячи короткоживущих
# клиентов подряд (свой SSL-контекст, буферы, TCP-хендшейк на каждый), что
